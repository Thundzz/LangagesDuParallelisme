...
def launch_bench_threads(max_threads)
	nb_iter = 3
	d_alphabet="abcdefghijklmnopqrstuvwxyz"
	d_nbp = 4
	d_depth = 6
	password = gen_random_pwd(1+d_depth)
	time = Array.new(max_threads, 0.0)
	iter_time = 0.0
	(2..max_threads).each do |d_nbt|
		for k in 1..nb_iter
			options = {}
			options = {alphabet: d_alphabet, nbp: d_nbp, nbt: d_nbt, depth: d_depth}.merge(options)
			cmd = "mpiexec -np 1 ./master.out  #{options[:nbp]} "+
			...
			File.open("time_#{d_nbp}_#{d_nbt}_#{d_depth}.dat", "r") do |file|
				file.each_line do |line|
					time[d_nbt] = line.to_s.split(" ")[0].to_f + time[d_nbt].to_f
				end
			end
		end
	end
